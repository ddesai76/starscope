"""
jira_ticket.py -- optional Jira integration for Perceptor.
...
-- no error, no missing-feature message, nothing.

Config lives in jira_config.json, next to this file, and is never
required to exist for perceptor.py itself to run -- only for a ticket
creation attempt to succeed. See CONFIG_TEMPLATE below for the shape.

Dependencies: stdlib only, matching perceptor.py's own constraint.
"""
